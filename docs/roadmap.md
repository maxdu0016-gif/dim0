...
- **Offline-first base — Phase 2 (drift-replace).** `writeInitialBase` seeds a v2 synced board's base locally only on the **pristine first connect** (`offline-first-data-model.md`); a reconnect **drift** snapshot is still applied in-memory only (base not replaced), and legacy (non-v2) synced boards keep no local replica at all. Replacing the base mid-session safely — swap it at a relay `serverSeq` while preserving interleaved unacked local edits — needs a **serverSeq-based** oplog truncation model (today's `writeSnapshot`/`foldBase` truncate by local seq). Unblocks: sourcing a synced board's sidebar hierarchy (and other reads) from the local store with a server fallback for never-opened boards. (#191.)
