...
- **Offline-first base — Phase 2 (drift-replace).** A v2 board's base is now materialized whole (all layers) via `materializeBoardOffline` + `?whole=true` (`offline-first-data-model.md`) whenever the replica is **fully synced** (no snapshot + no unsent local edit) — not only on a pristine first open. What's still deferred: (a) a board with **unsent local edits** at materialize time (folding the oplog would drop them — `materializeBoardOffline` defers); (b) a reconnect **drift** snapshot is still applied in-memory only (base not replaced); (c) legacy (non-v2) synced boards keep no local replica. Handling (a)/(b) safely — swap/rebuild the base while preserving interleaved *unsent* local edits — needs a **serverSeq-based** oplog truncation model (today's `foldBase` truncates by local seq, which is why we only fold an all-acked oplog). Unblocks: sourcing a synced board's sidebar hierarchy from the local store for never-opened boards. Remaining note from the #192 review:
