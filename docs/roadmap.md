...
- **Offline-first base — Phase 2 (drift-replace).** A v2 board's base is now materialized whole (all layers) on the **pristine first open** via `materializeBoardOffline` + `?whole=true` (`offline-first-data-model.md`); but a reconnect **drift** snapshot is still applied in-memory only (base not replaced), and legacy (non-v2) synced boards keep no local replica at all. Replacing the base mid-session safely — swap it at a relay `serverSeq` while preserving interleaved unacked local edits — needs a **serverSeq-based** oplog truncation model (today's `writeSnapshot`/`foldBase` truncate by local seq). Unblocks: sourcing a synced board's sidebar hierarchy (and other reads) from the local store with a server fallback for never-opened boards. Two known gaps that this same serverSeq model resolves (from the #192 review):
  - **Auto-seed lost to a peer-op race.** `writeInitialBase` bails when `this.seq !== 0`, which a relay `recordRemote` also bumps — so if a peer op lands during the in-flight whole-board fetch, an *online* open never materializes the base and the manual "download" then bails too (oplog non-empty). Seeding despite remote-only ops needs the fetch's serverSeq to truncate correctly (else replaying an op already in the base double-applies). Symptom: a busy collab board's subboards stay offline-unavailable; not data loss.
  - **`isBoardAvailableOffline` trusts any `snapshots` row = complete base.** Safe **today** (only `writeInitialBase`/`foldBase` write one, both complete, and `compact()` is unwired), but wiring compaction or any partial-state snapshot writer later must gate the offline marker on "whole-board seed ran", not mere row existence.
