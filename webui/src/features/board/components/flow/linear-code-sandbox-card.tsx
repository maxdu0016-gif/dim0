...
/**
 * Files-view card for Python sandbox notes with a lightweight icon preview and title editing.
 */
export const LinearCodeSandboxCard = memo(function LinearCodeSandboxCard({ node }: Props) {
  ...
  const displayTitle = node.data.label?.markdown?.trim() || 'Untitled sandbox'
  ...
  /**
   * Persist the edited title into the sandbox note label.
   */
  ...
  const handleOpenSurface = useCallback(() => {
    ...
    openNodeSurface(node.id, 'code-sandbox')
  ...
  return (
    <div className='group relative w-full min-w-0'>
      <button
        type='button'
        onClick={onDelete}
        className='absolute right-2 top-2 z-30 p-1 rounded-md text-white/70 hover:text-destructive transition-colors opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto'
      ...
      <button
        ...
        aria-label={`Open code sandbox ${displayTitle}`}
