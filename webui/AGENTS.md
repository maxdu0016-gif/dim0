...
- Desktop (Tauri) build? Branch on `isTauri()` (`@/platform`). Storage swaps IndexedDB → rusqlite (`persist/local/sqlite-engine.ts` + `src-tauri/storage.rs`), BYOK goes direct via plugin-http (`agent/engine/services/desktop-*.ts`), and managed/synced ride an optional server URL (`features/desktop/`). See `docs/adr/ADR-DESKTOP-001` + `ADR-DESKTOP-002`.
